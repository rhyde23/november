...
def fill_values(same_rating_games, distribution, expo_value) :
	...
	lv = values.index(2.0)
	for c, e in enumerate(values) :
		values[c] = round(e+lv, 3)
		if values[c] < 2.0 :
			values[c] = 2.0
		lv -= 1
	#print(values)
	file1 = open("morale_losses.txt","w")
	L = []
	start, end = 0, 69
...
def get_values() :
	file1 = open("morale_losses.txt","r").read()
	days_values = []
	current_array = []
	current_tr = 30
...
#same_rating_games, distribution, expo_value = 15, 40, 0.04
