...
def game(save_number) :
    ...
    def calculate_energy_deduction(stamina, age, max_considered_stamina, min_considered_stamina) :
        ...
        print('STAMINA DEDUCTION', (max_considered_stamina-stamina)/(max_considered_stamina-min_considered_stamina), stamina)
        age_deduction = age_deduction*((age-minimum_considered_age)/(maximum_considered_age-minimum_considered_age))
        print('AGE DEDUCTION', (age-minimum_considered_age)/(maximum_considered_age-minimum_considered_age), age)
                            ...
                            if current_data['StoppedOnMatchday'] :
                                ...
                                for user_player_not_playing in user_players :
                                    if user_player_not_playing in user_lineup :
                                        ...
                                        change = round(gain_multi*(in_lineup_expo_base**(calculate_rating(current_data[team+'_Players'], current_data[team+'_Lineup'], current_data[team+'_Formation'])-int(current_data[team+'_Players'][user_player_not_playing]['Rating'])+in_lineup_expo_x_adder)+in_lineup_expo_y_adder), 2)
                                        if change > 50.00 :
                                            change = 50.00
                                        ...
                                        change = round(loss_multi*(not_in_lineup_expo_base**(int(current_data[team+'_Players'][user_player_not_playing]['Rating'])-calculate_rating(current_data[team+'_Players'], current_data[team+'_Lineup'], current_data[team+'_Formation'])+not_in_lineup_expo_x_adder)), 2)
                                        if change > 25.00 :
                                            change = 25.00
                                        current_data[team+'_Players'][user_player_not_playing]['Morale'] -= change
                    ...
                    for league_progressing in leagues_in_game :
                        for team_progressing in current_data['TeamsInEachLeague'][league_progressing] :
                            ...
                            for player_progressing in current_data[key_progressing] :
                                ...
                                if current_data['WithoutYear'] == player_progress_bday :
                                ...
                                for name_of_stat in current_data[key_progressing][player_progressing]['DevelopmentPlan'+development_extension] :
                                    ...
                                    basis = None
                                    for ffnz_index, find_first_non_zero in enumerate(current_data[key_progressing][player_progressing]['DevelopmentPlan2'][name_of_stat]) :
                                        if find_first_non_zero != 0 :
                                            basis = (find_first_non_zero)*current_data[key_progressing][player_progressing]['DevelopmentPlanCountedDaysArray2'][ffnz_index]
                                            break
                                    dev_morale_change = (1-(current_data[key_progressing][player_progressing]['Morale']/expected_morale))*(basis/harshest_day_projection)
                                    if not morale_can_help :
                                        if dev_morale_change > 0 :
                                            dev_morale_change = 0
                                    #if team_progressing == 'Manchester United' :
                                        #print(player_progressing, current_data[key_progressing][player_progressing]['Morale'], dev_morale_change, name_of_stat)
                                    #print(dev_morale_change, 'DEV MORALE CHANGE FOR ', player_progressing, name_of_stat)
                                ...
                                if current_data[key_progressing][player_progressing]['DaysUntilNextUpdate'] == 8 :
                                    ...
                                    #get_new_change(player_morale, morale_at_default, importance_percentage, change)
                            ...
                            if current_data[team_progressing+'_LineupUpdate'] == 0 :
                                ...
                                current_data[team_progressing+'_LineupUpdate'] = 7
